...
export function isSameDevice(a, b) {
  const serialA = fold(a?.serialNumber);
  const serialB = fold(b?.serialNumber);
...
export function resolveScan({ serial, devices, picked }) {
  ...
  const chosen = Array.isArray(picked) ? picked : [];
  if (chosen.some((item) => fold(item?.serialNumber) === fold(scanned))) {
    return {
      ok: false,
      code: "duplicate",
      serial: scanned,
  ...
  const matches = (Array.isArray(devices) ? devices : []).filter(
    (device) => fold(device?.serialNumber) === fold(scanned)
  );
