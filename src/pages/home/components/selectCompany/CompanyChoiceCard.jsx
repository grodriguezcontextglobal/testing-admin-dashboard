...
const CompanyChoiceCard = ({ props }) => {
  ...
  return (
    <Link to="/"><Card onClick={() => handleSelect()} style={{ cursor: "pointer" }}>
      <Grid display={'flex'} flexDirection={'column'} padding={'12px 20px'} container >
        <Grid item xs sm md lg><Typography>{props.companyInfo.company_name}</Typography></Grid>
        <Grid display={'flex'} item xs sm md lg><Typography>{props.companyInfo.address.city},</Typography> <Typography>{props.companyInfo.address.state}</Typography></Grid>
      </Grid>
    </Card></Link>
  )
}
