...
const DeleteItemModal = ({ dataFound, openDeleteItemModal, setOpenDeleteItemModal }) => {
    ...
    const retrieveItemInfoForEdit = useCallback(() => {
        if (itemsInInventoryQuery.data) {
            ...
            setValue("ownership", `${itemsInInventoryQuery?.data?.data?.items[0]?.ownership}`)
            setValue("main_warehouse", `${itemsInInventoryQuery?.data?.data?.items[0]?.main_warehouse}`)
            setValue("location", `${itemsInInventoryQuery?.data?.data?.items[0]?.location}`)
        }
    ...
    const handleDeleteItem = async () => {
        ...
        try {
            ...
            const respAfterDelete = await devitrakApi.post(`/db_item/delete-item`, { item_id: device_id })
                ...
                for (let data of employees) {
                    if (data.role === "Administrator") {
                        const emailNotificationProfile = {
                            ...
                            message: `The device with serial number ${itemsInInventoryQuery?.data?.data?.items[0]?.serial_number} was deleted for staff member ${user.name} ${user.lastName} at Date ${new Date().toString()}`,
        ...
        return (<>
            ...
            >
                <Typography
                    ...
                    color={"var(--gray-600, #475467)"}
                >
                    You will delete this device from your company record. This action can not be reversed.
                </Typography>
            </InputLabel>
        </>
        )
    ...
    return (
        <Modal key={dataFound[0].item_id} open={openDeleteItemModal} onCancel={() => closeModal()} style={{ top: "20dv", zIndex: "7" }} width={1000} footer={[]}>
            <Grid
                display={"flex"}
