...
const EditItemModal = ({
  ...
  const [valueSelection, setValueSelection] = useState(
    (options.value = itemsInInventoryQuery?.data?.data?.items[0]?.ownership)
  );
  ...
  const retrieveItemInfoForEdit = useCallback(() => {
    if (itemsInInventoryQuery.data) {
      ...
      setLocationSelection(
        itemsInInventoryQuery?.data?.data?.items[0]?.warehouse
          ? dataFound[0]?.location
  ...
  useEffect(() => {
    ...
    itemsInInventoryQuery?.data?.data?.items[0]?.item_id,
    openEditItemModal,
    loadingStatusRendering,
  ]);
  ...
  if (itemsInInventoryQuery.isLoading)
    return (
