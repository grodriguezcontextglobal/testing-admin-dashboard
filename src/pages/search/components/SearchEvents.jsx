...
const SearchEvents = ({ searchParams }) => {
  ...
  const sortAndRenderFoundData = () => {
    if (staffMembersQuery.data) {
      ...
      const result = foundData?.filter(element => JSON.stringify(element).toLowerCase().includes(`${searchValue}`.toLowerCase()))
      return result
    }
  }
  useEffect(() => {
  ...
  const handleStoreData = async (record) => {
    ...
    dispatch(onAddEventData({ ...record.data, sql: sqpFetchInfo.data.events.at(-1) }));
    ...
    return (
      <Grid container style={{ display: "flex", justifyContent: "flex-start", alignItems: "center" }}>
        <Grid style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "center",alignSelf:"flex-start" }} item xs={12} sm={12} md={4} lg={4}>
          <Typography style={{ ...TextFontSize30LineHeight38, fontSize: "36px", lineHeight: "44px", fontWeight: 600, width: "100%", textAlign: "left" }}>Search events </Typography><br />
        ...
        <Grid item xs={12} sm={12} md={8} lg={8}>
          <Grid container gap={1}>
            {sortAndRenderFoundData()?.length > 0 ?
              sortAndRenderFoundData()?.map(item => <Grid key={item.id} item xs={12} sm={12} md={3} lg={3}> <CardEventsFound props={{ eventName: item.eventInfoDetail.eventName, address: item.eventInfoDetail.address, data: item }} fn={handleStoreData} /></Grid>)
              : <NoDataFound />
            }
          </Grid>
        </Grid>
