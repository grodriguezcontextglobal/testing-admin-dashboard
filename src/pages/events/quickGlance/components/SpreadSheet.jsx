...
const SpreadSheet = () => {
    ...
    useEffect(() => {
        ...
        if (transactionDeviceRecordInEvent.data && transactionPlusUserInfo.data) {
            ...
            setDefectedItems(transactionDeviceRecordInEvent.data.data.receiversInventory)
        }
        return () => {
            controller.abort()
        }
    ...
    const generateExcelFile = async () => {
        ...
        const wsData = [headers, ...sortAndGroupData().map(item => [
            item.firstName,
            item.lastName,
            item.email,
            item.phoneNumber,
        ...
        wsSheet1['!cols'] = [{ width: 20 }, { width: 20 }, { width: 30 }, { width: 20 }, { width: 30 }];
