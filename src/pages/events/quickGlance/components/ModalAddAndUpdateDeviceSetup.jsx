import { Button, Chip, Grid, InputLabel, OutlinedInput, Typography } from "@mui/material"
...
const ModalAddAndUpdateDeviceSetup = ({ openModalDeviceSetup, setOpenModalDeviceSetup, deviceTitle, quantity }) => {
    ...
    const itemQuery = useQuery({
        ...
        queryFn: () => devitrakApi.post("/db_item/warehouse-items", { company: user.company, warehouse: true, item_group: deviceTitle }),
        enabled: false,
        refetchOnMount: false
    });
    const recordNoSqlDevicesQuery = useQuery({
        queryKey: ["recordNoSqlDevices"],
        queryFn: () => devitrakApi.post("/receiver/receiver-pool-list", { provider: user.company, type: deviceTitle, eventSelected: event.eventInfoDetail.eventName }),
        enabled: false,
        refetchOnMount: false
    });
    ...
    const addingDeviceFromLocations = (data) => {
        if (recordNoSqlDevicesQuery?.data?.data?.receiversInventory.length === Number(quantity)) {
            return alert('Device type had reached out the quantity set when event was created.')
        } else {
            const checkingDiff = Number(quantity) - recordNoSqlDevicesQuery?.data?.data?.receiversInventory.length
            if (Number(data.quantity) > checkingDiff) {
                return alert(`Quantity assigned is bigger than needed to reach out the quantity set in event. Hint: ${checkingDiff} left.`)
            }
            const result = [...listOfLocations, { quantity: data.quantity, deviceInfo: valueItemSelected }]
    ...
    const blockingButton = () => {
        const initial = 0
        const checking = listOfLocations.reduce((accu, { quantity }) => accu + Number(quantity), initial)
        ...
        >
            ...
            >
                ...
                >
                    ...
                    >
                        <Typography
                            style={{ ...Subtitle, margin: "0px auto 1rem" }}
                        >
                            Enter serial number range for <strong>{deviceTitle}</strong> to assign to this event.
                        </Typography>
                                ...
                                options={optionsToRenderInSelector().map((item) => {
                                    return {
                                        label: <Typography textTransform={'capitalize'} style={{ ...Subtitle, display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                                            <span style={{ textAlign: "left", width: "50%" }}>Location: <span style={{ fontWeight: 700 }}>{item[0].location}</span></span>
                                            <span style={{ textAlign: "right", width: "5 0%" }}>Available: {item.length}</span>
                            ...
                            >
                                ...
                                >
                                    <Typography
                                        textTransform={"none"}
                                        textAlign={"left"}
                                        style={{ ...Subtitle, color: "transparent", fontWeight: 500, textWrap: "pretty" }}
                                    >
                                        Qty of devices from {valueItemSelected[0]?.location}
                                    </Typography>
                                </InputLabel>
                        ...
                        <div style={{ width: "100%", display: "flex", justifyContent: "flex-start", alignItems: "center", gap: "5px" }}>
                            {
                                listOfLocations.map((item, index) => {
                                    return (
                                        <Chip key={`${item.quantity}${item.deviceInfo.at(-1).item_id}`} label={`${item.deviceInfo.at(-1).location} - ${item.quantity}`} onDelete={() => removeItem(index)} />
                                    )
                                })
                            }
                        </div>
                    ...
                    {/* </Grid> */}
                    <Button
                        disabled={recordNoSqlDevicesQuery?.data?.data?.receiversInventory.length === Number(quantity)}
                        onClick={() => handleDevicesInEvent()}
                        style={{ ...disablingButton(), display: `${blockingButton() ? "flex" : "none"}`, width: "100%", }}
                    >
                        <Typography
                            textTransform={"none"}
