...
import { onAddDevicesSelection, onAddDevicesSelectionPaidTransactions } from "../../../../../../../../store/slices/devicesHandleSlice";
...
const MultipleDevices = ({ setCreateTransactionPaid }) => {
    ...
    const subtractRangePerGroupToDisplayItInScreen = useCallback(() => {
        ...
        if (result.length > 0) {
            return {
                max: formattingSerialNumberLeadingZero(max, deviceSelectionInfo.startingNumber),
                min: formattingSerialNumberLeadingZero(min, deviceSelectionInfo.startingNumber)
            }
        }
        return {
            max: 0,
    ...
    const generatePaymentIntent = async (data) => {
        ...
        if (response) {
            ...
            dispatch(onAddDevicesSelection((data.endingNumber - data.startingNumber)+1));
            dispatch(
                onAddDevicesSelectionPaidTransactions({ ...data, deviceType: JSON.parse(deviceSelection) }) //pass data serial number from handleSubmit
            );
        }
    };
    ...
    return (
        <>
            <form
                style={{ margin: " 0.5rem auto", display: `${clientSecret === "" ? "flex" : "none"}`, flexDirection: "column", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", width: "100%" }}
            ...
            >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1px", width: "100%" }}>
                    <OutlinedInput disabled={clientSecret !== ""} {...register("startingNumber")} autoFocus={true} style={{ ...OutlinedInputStyle, width: "90%" }} placeholder="Starting number" />
                    <OutlinedInput disabled={clientSecret !== ""} style={{ ...OutlinedInputStyle, width: "90%" }} type="text" placeholder="Ending number" {...register('endingNumber', { required: true })} />
                </div>
                <OutlinedInput disabled={clientSecret !== ""} style={{ ...OutlinedInputStyle }} type="text" placeholder="Amount to authorize." {...register('amount', { required: true })} fullWidth />
            ...
            {clientSecret !== "" && (
                <StripeElementChargeTransactionFromDashboard
