...
const Graphic = () => {
    ...
    const foundAllNoOperatingDeviceInEvent = useCallback(async () => {
        ...
        const checkDefectedDeviceList = await devitrakApi.post("/receiver/list-receiver-returned-issue", { "provider": user.company })
        if (checkDefectedDeviceList?.data?.ok) {
            const groupingByStatus = _.groupBy(checkDefectedDeviceList.data.record, "status")
        ...
        return setDefectedDeviceList(Array.from(finalResult).reduce((accu, curr) => accu + curr, 0))
    }, [])


    const lostDeviceInEvent = useCallback(async () => {
        const checkDefectedDeviceList = await devitrakApi.post("/receiver/list-receiver-returned-issue", { "provider": user.company, "status": "Lost" })
        if (checkDefectedDeviceList?.data?.ok) return setLostDeviceList(checkDefectedDeviceList.length)
        return 0
    }, []);
                ...
                return (
                    <Grid key={index} item xs={12}>
                        <FormatQuickGlanceCardGraphRender
                            key={index}
                            dataToRender={item}
                            totalDeviceInRange={itemInInventoryQuery?.data?.data?.items?.length ?? 0} //deviceRangeDisplay()
                            index={index}
                        />
                    </Grid>
                );
