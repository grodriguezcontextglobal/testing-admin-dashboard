...
const AssignStaffMemberToEvent = () => {
    ...
    const handleEventInfo = async (data) => {
        ...
        if (!JSON.parse(data.event).staff[data.role].some(element => element.email === profile.user)) {
            const newStaff = JSON.parse(data.event).staff[data.role] = [...JSON.parse(data.event).staff[data.role], {
                firstName: profile.firstName, lastName: profile.lastName, email: profile.user, role: dicRole[data.role]
            }]
            const keyParameter = `staff.${data.role}`
            const respAddingStaffMember = await devitrakApi.patch(`/event/edit-staff-event/${JSON.parse(data.event).id}`, {
                [keyParameter]: newStaff
            })
            if (respAddingStaffMember.data) {
                alert(`Staff member has been assigned to ${JSON.parse(data.event).eventInfoDetail.eventName}`)
                                        ...
                                        </MenuItem>
                                        {
                                            listOfEventsToRender.map(item => {
                                                return (
                                                    ...
                                                    >
                                                        <Typography>{item.eventInfoDetail.eventName}</Typography>
                                                    </MenuItem>
                                                )
                                            })
                                        }
