...
const InvitationLanding = () => {
    ...
    const warning = (type, content) => {
        messageApi.open({
            type: type,
            content: content,
            onClose: () => { setValue("password", ""); setValue("password2", "") },
        });
    };
    useEffect(() => {
    ...
    if (allStaffSavedQuery.data && companiesQuery.data) {
        ...
        const checkIfUserExistsInOtherCompany = () => {
            if (allStaffSavedQuery.data.data.adminUsers && allStaffSavedQuery.data.data.adminUsers.length > 0) {
                ...
                setValue("password2", displayMaskedPassword(allStaffSavedQuery.data.data.adminUsers.at(-1).password))
                return newUser.current = allStaffSavedQuery.data.data.adminUsers.at(-1)
            }
            return null
        }
        ...
        const updateExistingUser = async () => {
            const resp = await devitrakApi.patch(`/staff/edit-admin/${newUser.current.id}`, {
                multipleCompanies: true,
                companiesAssigned: [...newUser.current.companiesAssigned, {
                    company: company,
                    active: true,
                    super_user: false,
                    role: role
        ...
        const completeSubmitInfo = async (data) => {
            try {
                ...
                warning('success', 'Process completed successfully. Please go to log in to log in into your account.')
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {checkIfUserExistsInOtherCompany() !== null ? `Welcome back to devitrak App` : `Welcome to devitrak App`}
                                        </Typography>
                                        <Typography
                                            style={{
                                    ...
                                    >
                                        ...
                                        />
                                        <FormLabel style={{ marginBottom: "0.5rem" }}>
                                            <Typography style={Subtitle}> You need to enter a password to complete registration process with the company invitation.</Typography>
                                        </FormLabel>
                                    </Grid>
                                    ...
                                    >
                                        ...
                                        <OutlinedInput
                                            disabled={loadingStatus || checkIfUserExistsInOtherCompany() !== null}
                                            {...register("password", { minLength: 6, value: `${checkIfUserExistsInOtherCompany() !== null ? setValue('password', checkIfUserExistsInOtherCompany().password) : ""}` })}
                                            style={OutlinedInputStyle}
                                            placeholder="******"
                                            type="password"
                                            fullWidth
                                        />
                                        {errors?.password && <p style={{ ...Subtitle, color: "var(--main-colorslobster)" }}>{errorDic[errors.password.type]}</p>}
                                    </Grid>
                                    <Grid
                                        marginY={"20px"}
                                        marginX={0}
                                    ...
                                    >
                                        ...
                                        <OutlinedInput
                                            disabled={loadingStatus || checkIfUserExistsInOtherCompany() !== null}
                                            {...register("password2", { minLength: 6, value: `${checkIfUserExistsInOtherCompany() !== null ? setValue('password2', checkIfUserExistsInOtherCompany().password) : ""}` })}
                                            style={OutlinedInputStyle}
                                            placeholder="******"
                                            type="password"
                                            fullWidth
                                        />
                                        {errors?.password2 && <p style={{ ...Subtitle, color: "var(--main-colorslobster)" }}>{errorDic[errors.password2.type]}</p>}
                                    </Grid>
                                    ...
                                    <Grid
                                        ...
                                        xs={12}
                                    >
                                        <Button disabled={loadingStatus} htmlType="submit" style={{ ...BlueButton, width: "100%" }}>
                                            <Typography style={{ ...BlueButtonText, margin: "auto" }}>Submit registration</Typography>
                                        </Button>
                                    </Grid>
                                    <Grid
                                        item
