...
const Registration = () => {
  ...
  const watchEmailConfirmation = watch("email_confirmation");

  const showFullForm = emailCheckDone && userExists.length === 0;

  // Resetea el estado cuando los emails cambian o dejan de coincidir
  useEffect(() => {
    const emailsMatch = watchEmail && watchEmailConfirmation && watchEmail === watchEmailConfirmation;
    const emailValid  = isValidEmail(watchEmail);

    if (!emailsMatch || !emailValid) {
      setUserExists([]);
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const response = await devitrakApi.post("/staff/__staff-search", { email: watchEmail });
  ...
  // Usuario existente: dispatch automático y navegar a company-setup
  useEffect(() => {
    if (!emailCheckDone || userExists.length === 0) return;
  ...
  }, [emailCheckDone, userExists.length]);
  ...
  const onSubmitRegister = async (data) => {
    if (data.password !== data.password2)
      return openNotificationWithIcon("error", "Action denied.", "Password must match.");
    try {
      let base64 = "";
      ...
      openNotificationWithIcon("error", "Action was not accepted. Please try again later.", `${error.response}`);
    }
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* ── Resto del formulario: solo para usuario nuevo ── */}
            {showFullForm && (
              <>
                <Grid marginY={"20px"} marginX={0} textAlign={"left"} item xs={12}>
                  <FormLabel style={{ marginBottom: "0.5rem" }}>
                ...
                <Grid display={"flex"} flexDirection={"column"} justifyContent={"flex-start"} alignItems={"center"} marginY={0} gap={2} item xs={12} sm={12} md={12}>
                  ...
                  <Grid display={"flex"} justifyContent={"flex-start"} alignSelf={"stretch"} marginY={0} gap={2} item xs={12} sm={12} md={12}>
                    <Grid display={"flex"} justifyContent={"flex-start"} alignSelf={"stretch"} marginY={0} gap={2} item xs={4} sm={4} md={4}>
                      <Avatar
                        size={{ xs: 24, sm: 32, md: 40, lg: 64, xl: 80, xxl: 100 }}
                        src={user.data ? user.data.imageProfile : <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><UploadImagePlaceholder /></div>}
                        style={{ background: "var(--Gray-100, #F2F4F7)" }}
                      />
                    </Grid>
                    <Grid display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"center"} marginBottom={2} style={{ width: "100%", borderRadius: "12px", border: "1px solid var(--gray-200, #EAECF0)", background: "var(--base-white, #FFF)" }} item xs={12}>
                      <Grid display={"flex"} justifyContent={"center"} alignItems={"center"} marginTop={2} item xs={12}>
                        <Avatar style={{ display: "flex", justifyContent: "center", alignItems: "center", border: "6px solid var(--gray-50, #F9FAFB)", background: "6px solid var(--gray-50, #F9FAFB)", borderRadius: "28px" }}>
                          <Icon icon="tabler:cloud-upload" color="#475467" width={20} height={20} />
                        </Avatar>
                      </Grid>
                      <Grid display={"flex"} justifyContent={"center"} alignItems={"center"} item xs={12} sm={10}>
                        <TextField {...register("photo")} id="file-upload" type="file" className="photo_input" accept=".jpeg, .png, .jpg" style={{ outline: "none", border: "transparent" }} />
                      </Grid>
                      <Grid display={"flex"} justifyContent={"center"} alignItems={"center"} marginBottom={2} item xs={12}>
                        <p style={{ ...Subtitle, fontWeight: 400 }}>SVG, PNG, JPG or GIF (max. 1MB)</p>
                      </Grid>
                    </Grid>
                  </Grid>
                </Grid>
                ...
                <Grid marginY={"20px"} marginX={0} textAlign={"left"} display={"flex"} justifyContent={"space-between"} alignItems={"center"} item xs={12}>
                  <BlueButtonComponent buttonType="submit" title="Set up new company" styles={{ width: "100%" }} />
                </Grid>
              </>
            )}
          </form>
